...
type Devices []struct {
	Identifier string `json:"baUUID,omitempty"`
	...
	BatteryStatus float64 `json:"batteryLevel,omitempty"`
	// Address    struct {
...
func ParseDevices() (Devices, error) {
	u, err := user.Lookup(os.ExpandEnv("$USER"))
	if err != nil {
		return nil, fmt.Errorf("user lookup: %+v", err)
	}

	devicesPath := fmt.Sprintf("%s/Library/Caches/com.apple.findmy.fmipcore/Devices.data", u.HomeDir)

	// stat, errStat := os.Stat(devicesPath)
	// if errStat != nil {
	...
	// fmt.Println(devicesPath, "updated at", lastModTime)

	// Open devices file
	f, errReadFile := os.ReadFile(devicesPath)
	if errReadFile != nil {
	...
	// Parse devices
	errUnmarshal := json.Unmarshal(f, &devices)
	if errUnmarshal != nil {
